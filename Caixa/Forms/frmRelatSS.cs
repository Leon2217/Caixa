...
{
    public partial class frmRelatSS : Form
    {
        ...
        private void frmRelatGDS_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frmRelatGDS_Load(object sender, EventArgs e)
        {
            try
            {
                gvExibir.DataSource = vgdsDAO.ListarTudo();
        ...
        private void mskData_TextChanged(object sender, EventArgs e)
        {
            if (mskData.MaskFull == false)
            {
                ...
                try
                {
                    vgdsDAO.VerificaTotalLabelTudo();
                    total = Convert.ToDouble(vgdsDAO.Vgds.Total_gds.ToString().Replace(".", ""));
            ...
            else
            {
                ...
                gvExibir.DataSource = vgdsDAO.ListarSemana(data);
                try
                {
                    vgdsDAO.VerificaTotalLabel(data);
                    total = Convert.ToDouble(vgdsDAO.Vgds.Total_gds.ToString().Replace(".", ""));
                    lblTotalV.Text = total.ToString("C2");
                }
                catch
